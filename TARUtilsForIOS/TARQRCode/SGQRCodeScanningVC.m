...
- (void)manager:(SGQRCodeManager *)manager captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        /*
        ScanSuccessJumpVC *jumpVC = [[ScanSuccessJumpVC alloc] init];
        jumpVC.jump_URL = stringValue;
        [self.navigationController pushViewController:jumpVC animated:YES];
        */
        
        [self scanJumpResult:stringValue];
    } else {
        NSLog(@"暂未识别出扫描的二维码");
    }
}
...
-(void)scanJumpResult:(NSString *)result
{
    [self.navigationController popViewControllerAnimated:YES];
    if (self.delegate && [self.delegate respondsToSelector:@selector(ScanCompleteJumpProtocol:)]) {
        [self.delegate ScanCompleteJumpProtocol:result];
    }
}
