...
//（小红点）对象集合
@property(nonatomic,strong)NSMutableArray *smallRedDots;
...
-(void)loadDataAndView
{
    ...
    self.smallRedDots = [[NSMutableArray alloc]init];
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    for (NSString *title in titles)
    {
        ...
        CGFloat eachButtonWidth = size.width + 30.f;
...
-(void)initSmallRedDot
{
    for (NSUInteger index = 0; index < _items.count; index++)
    {
        if (![[_items objectAtIndex:index] isKindOfClass:[UIButton class]]) {
            return;
        }
        UIButton *butotn = [_items objectAtIndex:index];
        
        CGFloat smallRedDot_W_H = 8;
        UIView *smallRedDot = [[UIView alloc]initWithFrame:CGRectMake(butotn.width-(smallRedDot_W_H+3), 3, smallRedDot_W_H, smallRedDot_W_H)];
        smallRedDot.hidden = YES;
        smallRedDot.layer.cornerRadius = smallRedDot_W_H/2.0;
        smallRedDot.backgroundColor = [UIColor redColor];
        [butotn addSubview:smallRedDot];
        
        [_smallRedDots addObject:smallRedDot];
    }
}

- (void)setSmallRedDotShowArray:(NSArray <NSNumber *> *)showArray isShow:(BOOL)yesOrNo
{
    _isShowSmallRedDot = yesOrNo;
    if (_isShowSmallRedDot == YES) {
        for (NSUInteger index = 0; index < _smallRedDots.count; index++)
        {
            if (![[_smallRedDots objectAtIndex:index] isKindOfClass:[UIView class]]) {
                return;
            }
            UIView *view = [_smallRedDots objectAtIndex:index];
            BOOL isShow = NO;
            if (showArray.count > index) {
                isShow = [[showArray objectAtIndex:index] boolValue];
            }
            if (isShow == YES) {
                view.hidden = NO;
            }else{
...
-(void)setMenuItemArray:(NSArray<MenuItemModel *> *)menuItemArray
{
    _menuItemArray = menuItemArray;
    
    NSMutableArray *myTitleArray = [[NSMutableArray alloc] init];
    for (int i=0; i<menuItemArray.count; i++) {
        MenuItemModel *model = [menuItemArray objectAtIndex:i];
        NSString *myTitle = model.menuItemName;
        [myTitleArray addObject:myTitle];
    }
    [self setMyTitleArray:myTitleArray];
}
...
-(void)curSelectedIndex:(NSInteger)index
{
    ...
    if ([self.delegate respondsToSelector:@selector(itemDidSelectedWithIndex:withSourceDatas:)]) {
        [self.delegate itemDidSelectedWithIndex:index withSourceDatas:_menuItemArray];
    }
